...
class Superpoint(BaseModel):
    ...
    def _forward(self, data):
        ...
        if self.config['has_detector'] and ('t' in tsp):
            ...
            cPa = torch.nn.functional.interpolate(cPa, size=(self.config['height']//8,self.config['width']//8), mode='bilinear',align_corners=self.config['align_corners'])
        ...
        if self.config['has_descriptor'] and ('s' in tsp): 
            ...
            cDa = torch.nn.functional.interpolate(cDa, size=(self.config['height']//8,self.config['width']//8), mode='bilinear',align_corners=self.config['align_corners'])
